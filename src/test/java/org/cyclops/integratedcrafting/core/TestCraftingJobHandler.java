...
public class TestCraftingJobHandler {
    ...
    public void testSerializationDoesNotGrowWithRecipes() {
        handler.reportRecipeDuration(recipeA, 100);
        TagValueOutput valueOutputSingle = createOutput();
        handler.serialize(valueOutputSingle);
        CompoundTag tagSingle = valueOutputSingle.buildResult();

        for (int i = 0; i < 100; i++) {
            handler.reportRecipeDuration(newRecipe(i), 100);
        }
        TagValueOutput valueOutputMany = createOutput();
        handler.serialize(valueOutputMany);
        CompoundTag tagMany = valueOutputMany.buildResult();
    ...
    public void testRecipeDurationsSurviveSerialization() {
        ...
        TagValueOutput valueOutput = createOutput();
        handler.serialize(valueOutput);

        TickingCraftingJobHandler deserialized = new TickingCraftingJobHandler();
        deserialized.deserialize(TagValueInput.create(ProblemReporter.DISCARDING,
    ...
    protected static TagValueOutput createOutput() {
