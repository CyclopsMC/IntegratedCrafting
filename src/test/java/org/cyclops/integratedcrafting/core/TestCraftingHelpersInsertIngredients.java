...
public class TestCraftingHelpersInsertIngredients {
    ...
    private static class CappedStorage implements IIngredientComponentStorage<ComplexStack, Integer> {
        ...
        private final SnapshotJournal<int[]> journal = new SnapshotJournal<>() {
            @Override
            protected int[] createSnapshot() {
                return new int[]{freeSpace, accepted};
            }

            @Override
            protected void revertToSnapshot(int[] snapshot) {
                freeSpace = snapshot[0];
                accepted = snapshot[1];
            }
        };
        ...
        @Override
        public ComplexStack insert(@Nonnull ComplexStack instance, TransactionContext transaction) {
        ...
        @Override
        public ComplexStack extract(@Nonnull ComplexStack instance, Integer matchCondition, TransactionContext transaction) {
            return null;
        }

        @Override
        public ComplexStack extract(long maxQuantity, TransactionContext transaction) {
            return null;
        }
    }
