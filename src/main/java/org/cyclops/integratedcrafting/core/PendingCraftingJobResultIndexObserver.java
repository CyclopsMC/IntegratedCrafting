...
public class PendingCraftingJobResultIndexObserver<T, M> implements IIngredientChannelInsertPreConsumer<T> {
    ...
    @Override
    public T insert(int channel, @NotNull T ingredient, TransactionContext transaction) {
        IngredientInstanceWrapper<T, M> instanceWrapper = addIngredient(new IngredientInstanceWrapper<>(ingredientComponent, ingredient), channel, true);
        new RootCommitJournal(() -> addIngredient(new IngredientInstanceWrapper<>(ingredientComponent, ingredient), channel, false)).updateSnapshots(transaction);
