...
public class PendingCraftingJobResultIndexObserver<T, M> implements IIngredientChannelInsertPreConsumer<T> {
     ...
     *
     * Only the part of the instance that is still unclaimed may resolve pending outputs,
     ...
     * @param unclaimed The part of the instance that no crafting interface has claimed yet.
     ...
     *         and the part of it that is still unclaimed.
     */
    public IIngredientChannelInsertPreConsumer.Result<T> addIngredient(T instance, T unclaimed, int channel, boolean simulate) {
        IIngredientMatcher<T, M> matcher = ingredientComponent.getMatcher();
        long instanceAmount = matcher.getQuantity(instance);
        long unclaimedAmount = matcher.getQuantity(unclaimed);
        ...
        while (jobsEntryIt.hasNext() && unclaimedAmount > 0) {
            ...
            if (craftingJob != null
                ...
                while (jobEntryIt.hasNext() && unclaimedAmount > 0) {
                    ...
                    if (pendingIngredientsUnsafe != null) {
                        ...
                        while (it.hasNext() && unclaimedAmount > 0) {
                            ...
                            if (matcher.matches(instance, prototypedIngredient.getPrototype(),
                                ...
                                // Claim this part of the insertion for this pending output,
                                // so that no other crafting interface can resolve its own job with it.
                                // It is not taken away: the output still has to reach the network storage.
                                long claimedQuantity = Math.min(remainingQuantity, unclaimedAmount);
                                remainingQuantity -= claimedQuantity;
                                unclaimedAmount -= claimedQuantity;
                                ...
                                long extractedQuantityToAssign = Math.min(claimedQuantity, instanceAmount);
        ...
        return new IIngredientChannelInsertPreConsumer.Result<>(matcher.withQuantity(instance, instanceAmount),
                matcher.withQuantity(instance, unclaimedAmount));
    }
    ...
    public IIngredientChannelInsertPreConsumer.Result<T> insert(int channel, @NotNull T ingredient,
                                                                @NotNull T unclaimed, TransactionContext transaction) {
        // The simulated pass is what the next pre-consumer sees, so it decides what is claimed here.
        // The committed pass re-runs with those same inputs, and therefore claims exactly the same.
        IIngredientChannelInsertPreConsumer.Result<T> result = addIngredient(ingredient, unclaimed, channel, true);
        new RootCommitJournal(() -> addIngredient(ingredient, unclaimed, channel, false)).updateSnapshots(transaction);
        return result;
    }
}
