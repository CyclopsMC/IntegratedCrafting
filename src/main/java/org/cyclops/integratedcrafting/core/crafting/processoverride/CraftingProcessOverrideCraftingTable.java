...
public class CraftingProcessOverrideCraftingTable implements ICraftingProcessOverride {
    ...
    public boolean craft(Function<IngredientComponent<?, ?>, PartPos> targetGetter,
        ...
        return IModHelpers.get().getCraftingHelpers().findRecipeCached(RecipeType.CRAFTING, gridInput, level, false)
                .or(() -> {
                    try {
                        CraftingGrid gridSmall = new CraftingGrid(ingredients, 2, 2);
                        return IModHelpers.get().getCraftingHelpers().findRecipeCached(RecipeType.CRAFTING, gridSmall.asCraftInput(), level, false);
