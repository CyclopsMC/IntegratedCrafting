...
public class CraftingProcessOverrideSmithingTable implements ICraftingProcessOverride {
    ...
    public boolean craft(Function<IngredientComponent<?, ?>, PartPos> targetGetter,
        ...
        return IModHelpers.get().getCraftingHelpers().findServerRecipe(RecipeType.SMITHING, smithingRecipeInput, level)
                .map(recipeHolder -> {
                    ...
                    if (!simulate) {
                        ...
                        for (ItemStack remainingItem : smithingRecipe.getRemainingItems(smithingRecipeInput)) {
