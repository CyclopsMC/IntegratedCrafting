...
public class GameTestsPerformance {
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_interfaces_crafting_idle")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_interfaces_crafting_idle_recipes")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_craft_satisfied_idle")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_craft_simple")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_craft_nested")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_craft_recipe_index")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_interfaces_crafting_append")
    ...
    @GameTest(template = TEMPLATE_EMPTY, timeoutTicks = TIMEOUT_TICKS, batch = "performance_interfaces_crafting_remove")
    ...
    public static void testPerformance(GameTestHelper helper, String networkName, Consumer<Runnable> networkConstructor) {
        ...
        helper.runAfterDelay(WARMUP_TICKS, () -> {
            ...
            if (!problems.isEmpty()) {
                throw new GameTestAssertException("Preset " + networkName + " generated " + problems.size()
                        + " broken parts, first: " + problems.get(0));
            }
        });
