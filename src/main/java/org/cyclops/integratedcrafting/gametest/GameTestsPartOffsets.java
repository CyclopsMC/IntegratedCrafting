...
public class GameTestsPartOffsets {
    ...
    public void testItemsCraftChestOffset(GameTestHelper helper) {
        ...
        setPartOffset(helper, positions.interfaces().get(0), new Vec3i(0, 0, -1));
    ...
    public void testItemsCraftAttunedPlanksOffset(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            // Check crafting interface state
            helper.assertTrue(positions.interfaceStates().get(0).hasValidTarget(), Component.literal("Crafting interface has no valid target"));

            // Check if items have been crafted
            helper.assertValueEqual(chestIn.getItem(0).getItem(), Items.OAK_LOG, Component.literal("Slot 0 item is incorrect"));
            ...
            helper.assertValueEqual(chestIn.getItem(1).getCount(), 4, Component.literal("Slot 1 amount is incorrect"));
        });
    }
