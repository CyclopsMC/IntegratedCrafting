...
public class GameTestsAttunedRecipes {
    ...
    protected static boolean containsRecipeId(Collection<IRecipeDefinition> recipes, String recipeId) {
        return recipes.stream()
                .anyMatch(recipe -> recipe.getRecipeId() != null && recipe.getRecipeId().identifier().toString().equals(recipeId));
    }
    ...
    protected static int indexOfRecipeId(List<IRecipeDefinition> recipes, String recipeId) {
        for (int i = 0; i < recipes.size(); i++) {
            ...
            if (recipe.getRecipeId() != null && recipe.getRecipeId().identifier().toString().equals(recipeId)) {
                return i;
            }
        }
        return -1;
    ...
    public void testAttunedDisabledRecipesArePersisted(GameTestHelper helper) {
        ...
        helper.startSequence()
                ...
                .thenExecute(() -> {
                    partState.setRecipesEnabled(List.of(chestKey, removedKey), false);

                    TagValueOutput valueOutput = TagValueOutput.createWithContext(ProblemReporter.DISCARDING,
                    ...
                    loadedState.deserialize(TagValueInput.create(ProblemReporter.DISCARDING,
                            helper.getLevel().registryAccess(), valueOutput.buildResult()));
