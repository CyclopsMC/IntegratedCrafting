    ...
    public void testItemsCraftEnumeratedChest(GameTestHelper helper) {
        ...
        positions.interfaceStates().get(0).getInventoryVariables().setItem(0,
                createVariableForRecipeList(helper.getLevel(), List.of(
                        Pair.of(RecipeType.CRAFTING, Identifier.fromNamespaceAndPath("minecraft", "chest")),
                        Pair.of(RecipeType.CRAFTING, Identifier.fromNamespaceAndPath("minecraft", "crafting_table"))
                )));
    ...
    public void testItemsCraftEnumeratedReplaceVariable(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            if (!replaced[0]) {
                ...
                replaced[0] = true;
                throw new GameTestAssertException(Component.literal("Waiting for the replaced variable to be picked up"), (int) helper.getTick());
            }
    ...
    public void testItemsCraftEnumeratedMachineReader(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            if (!targetRemoved[0]) {
                ...
                throw new GameTestAssertException(Component.literal("Waiting for the invalidated variable to be picked up"), (int) helper.getTick());
            }
