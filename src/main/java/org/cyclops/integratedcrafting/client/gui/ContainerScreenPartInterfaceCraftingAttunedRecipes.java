...
public class ContainerScreenPartInterfaceCraftingAttunedRecipes extends ContainerScreenScrolling<ContainerPartInterfaceCraftingAttunedRecipes> {
    ...
    protected Identifier constructGuiTexture() {
        return Identifier.fromNamespaceAndPath(Reference.MOD_ID, "textures/gui/part_interface_crafting_attuned_recipes.png");
    }
    ...
    protected int getGridWidth() {
        return getMenu().getColumns() * IModHelpers.get().getGuiHelpers().getSlotSize();
    }

    protected int getGridHeight() {
        return getMenu().getPageSize() * IModHelpers.get().getGuiHelpers().getSlotSize();
    }
    ...
    protected int getCellIndexAt(double mouseX, double mouseY, boolean skipCellBorders) {
        ...
        if (skipCellBorders && (relativeX % IModHelpers.get().getGuiHelpers().getSlotSize() >= IModHelpers.get().getGuiHelpers().getSlotSizeInner()
                || relativeY % IModHelpers.get().getGuiHelpers().getSlotSize() >= IModHelpers.get().getGuiHelpers().getSlotSizeInner())) {
            return -1;
        }
        return relativeX / IModHelpers.get().getGuiHelpers().getSlotSize()
                + (relativeY / IModHelpers.get().getGuiHelpers().getSlotSize()) * getMenu().getColumns();
    }

    protected int getCellX(int index) {
        return this.leftPos + this.offsetX + GRID_X + (index % getMenu().getColumns()) * IModHelpers.get().getGuiHelpers().getSlotSize();
    }

    protected int getCellY(int index) {
        return this.topPos + this.offsetY + GRID_Y + (index / getMenu().getColumns()) * IModHelpers.get().getGuiHelpers().getSlotSize();
    }
    ...
    public boolean mouseDragged(MouseButtonEvent mouse, double dragX, double dragY) {
        if (this.draggedRecipes != null && mouse.button() == 0) {
            ...
            for (int step = 1; step <= steps; step++) {
                applyDragAt(mouse.x() - dragX * (steps - step) / steps, mouse.y() - dragY * (steps - step) / steps);
            }
            return true;
        }
    ...
    public void extractBackground(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        IModHelpers.get().getRenderHelpers().drawScaledCenteredString(guiGraphics, font,
                ...
                ARGB.opaque(4210752), false, Font.DisplayMode.NORMAL);
        ...
        for (int i = 0; i < cells; i++) {
            ...
            guiGraphics.outline(x, y, IModHelpers.get().getGuiHelpers().getSlotSizeInner(), IModHelpers.get().getGuiHelpers().getSlotSizeInner(),
                    enabled ? COLOR_BORDER_ENABLED : COLOR_BORDER_DISABLED);
            if (IModHelpers.get().getRenderHelpers().isPointInRegion(x, y, IModHelpers.get().getGuiHelpers().getSlotSizeInner(), IModHelpers.get().getGuiHelpers().getSlotSizeInner(),
                    mouseX, mouseY)) {
                guiGraphics.fill(x, y, x + IModHelpers.get().getGuiHelpers().getSlotSizeInner(), y + IModHelpers.get().getGuiHelpers().getSlotSizeInner(), COLOR_HOVER);
            }

            ItemStack outputItem = ContainerPartInterfaceCraftingAttunedRecipes.getOutputItem(recipe);
            if (!outputItem.isEmpty()) {
                guiGraphics.item(outputItem, x, y);
                guiGraphics.itemDecorations(font, outputItem, x, y);
            }

            if (!enabled) {
                // Drawn after the output, so that it covers it
                guiGraphics.fill(x, y, x + IModHelpers.get().getGuiHelpers().getSlotSizeInner(),
                        y + IModHelpers.get().getGuiHelpers().getSlotSizeInner(), COLOR_DISABLED);
            }
        }
    }
    ...
    protected void renderRecipeTooltip(GuiGraphicsExtractor guiGraphics, IRecipeDefinition recipe, int mouseX, int mouseY) {
        ...
        // Tooltips are drawn on top of everything else at the end of the frame.
        guiGraphics.setComponentTooltipFromElementsForNextFrame(font, elements, mouseX, mouseY, ItemStack.EMPTY);
    }
