...
2. **Game Tests** (`src/main/java/org/cyclops/integratedcrafting/gametest/GameTestsPerformance.java`)
   ...
   - Writes results to `run/logs/benchmark_results.txt`
...
3. Add a new `@GameTest` method in `GameTestsPerformance`, with a unique `environment` name,
   so that the benchmark runs in isolation from the other benchmarks,
   and add a matching empty environment definition under
   `src/main/resources/data/integratedcrafting/test_environment/`
